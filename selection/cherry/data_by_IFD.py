...
def main():
    parser = HfArgumentParser(SiftingByIFDArguments)
    args: SiftingByIFDArguments = parser.parse_args_into_dataclasses()[0]
    ...
    for i in tqdm(range(len(pt_data))):
        ...
        if args.prompt == 'wiz':
            ...
            if input_i == '':
                ...
                promt_to_use = LLAMA_ALPACA_PROMPT_DICT["prompt_no_input"].format_map(temp_dict)
                whole_text = promt_to_use + output_i
                instruct_i = promt_to_use
            else:
                temp_dict = {'instruction': instruct_i, 'input': input_i}
                promt_to_use = LLAMA_ALPACA_PROMPT_DICT["prompt_input"].format_map(temp_dict)
                whole_text = promt_to_use + output_i
                instruct_i = promt_to_use
        ...
        if args.max_length - instruct_i_len > 0:
            ...
            len_2, token_ids_2, loss_list_2 = get_loss_part_text(tokenizer, whole_text, output_i, args.max_length,
                                                                 loss_2_list)
    ...
    new_data_indexes = mean_rate_list_id_sample
    print('New data len \n', len(new_data))
    with open(args.json_save_path, "w") as fw:
        json.dump(new_data, fw, indent=4)
    with open(args.json_save_path.replace('.json', '_indexes.json'), "w") as fw:
        json.dump(new_data_indexes, fw, indent=4)
