...
class Config(ABC):
    ...
    # def _pop_unused_args(self, args_dict):
    #     for key in pop_config_name:
    #         if key in args_dict:
    #             args_dict.pop(key)
    #     return args_dict
    ...
    def config_check_tuning(self):
        ...
        if not self.M.tuning_type or "fine" in self.M.tuning_type:
            ...
            # self.M.tuning_type = ""
        # elif is_petuning(self.M.tuning_type):
    ...
    def save_configs(self, save_path):
        # save_configs = OmegaConf.create()
        # save_configs["data_config"] = self._pop_unused_args(dict(vars(self.D)))
        # save_configs["federated_config"] = self._pop_unused_args(dict(vars(self.F)))
        # save_configs["model_config"] = self._pop_unused_args(dict(vars(self.M)))
        # save_configs["training_config"] = self._pop_unused_args(dict(vars(self.T)))
        # OmegaConf.save(save_configs, save_path)
        ...
...
def amend_config(model_args, data_args, training_args, federated_args):
    ...
    if phase == "train" and config.T.local_rank <= 0 and not config.data_config.debug_mode and config.T.do_grid:
        config.save_configs(config.T.yaml_file)
